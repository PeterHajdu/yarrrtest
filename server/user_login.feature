...
Feature: User login
  ...
  Scenario: The server allows existing users login
    ...
    When I start a client with command line parameters --username known_user --auth_token dogfood
  ...
  Scenario: The server does not allow non existing user login.
    Given a running server
    When I start a client with command line parameters --username unknown_user --auth_token dogfood
    And I gather model information
  ...
  Scenario: The server does not allow existing user login without the correct authentication token.
    Given a running server
    And a registered user "known_user" with auth_token "dogfood"
    When I start a client with command line parameters --username known_user --auth_token catbeverage
    And I gather model information
